...
A command is an entry in a plugin's `plugin.jsonc` (see [Plugins](04-plugins.md) — installing one
is a `git clone`). It has a `name`, optionally a `subtitle` and an `icon`, and exactly one of
three things it does.

## run

A shell one-liner. It goes to `/bin/sh -c` with the plugin's own folder as the working directory, so `open` and `./script.sh` both work as written — `./script.sh` means the script sitting next to the manifest.
...
```jsonc
{ "name": "GitHub", "prefix": "gh ", "icon": "globe", "menu": "./gh-search \"$1\"" },
```

Typing `gh swift` runs `./gh-search "swift"` and lists what it printed.
