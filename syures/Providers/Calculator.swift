...
struct Calculator: Provider {
    ...
    static func evaluate(_ query: String) -> Launcher.Provided? {
        ...
        return Launcher.Provided(name: "= \(text)", subtitle: expression,
                                 icon: .symbol("equal.square"), action: .copy(text))
    }
