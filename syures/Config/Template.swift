...
extension Config {
    static let template = """
    {
      ...
      // There is no "commands" key: everything besides the installed apps comes from plugins.
      // A plugin is a folder under plugins/ next to this file — usually a `git clone` — whose
      // plugin.jsonc is an array of commands; see plugins/starter/plugin.jsonc for the shape.
      // Installing is cloning, updating is pulling, removing is deleting the folder.
    }
    """
}
...
extension Config {
    ...
    static let schema = """
    {
      ...
      "title": "syures config",
      "description": "Generated by syures on launch. Edits here are overwritten. Also validates a plugin manifest (plugins/<name>/plugin.jsonc) and the command array a menu script prints.",
      "oneOf": [
        { "$ref": "#/$defs/config" },
        { "$ref": "#/$defs/manifest" },
      ...
      "properties": {
        ...
        "$comment": "Commands live in plugins/<name>/plugin.jsonc, not here."
      }
        },
        "manifest": {
          "type": "object",
          ...
          "properties": {
            ...
            "commands": {
              "type": "array",
              "description": "The plugin's commands, shown alongside installed apps.",
...
extension Config {
    /// The starter plugin, written next to the config on first run: the shape of a manifest, the
    /// install/update commands, and a few examples to steal. A plugin's scripts run from its own
    /// folder, which is why the paths below climb two levels to reach the config.
    static let starterPlugin = """
    {
      "$schema": "../../config.schema.json",

      // The manifest's one key: an array of commands, the same shape a `menu` script prints.
      "commands": [
      // { "name": "Sleep", "icon": "moon.fill", "run": "pmset sleepnow" },     // SF Symbol
      // { "name": "Files", "icon": "\u{f07b}", "run": "open ~" },              // any text works too

      { "name": "Install Plugin", "prefix": "install ", "icon": "arrow.down.circle",
        "subtitle": "install <git url> — a plugin is anything you can clone",
        "run": "git -C .. clone \\"$1\\"" },
      { "name": "Update Plugins", "icon": "arrow.clockwise",
        "subtitle": "git pull in every plugin folder",
        "run": "for d in ../*/; do git -C \\"$d\\" pull -q 2>/dev/null; done; true" },
